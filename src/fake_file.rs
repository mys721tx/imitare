...
mod tests {
    ...
    fn test_extension_headers() {
        ...
        assert_eq!(pdf_header[0..8], [0x25, 0x50, 0x44, 0x46, 0x2d, 0x31, 0x2e, 0x34]);

        // DOC header test - should match compound document signature
        let doc_header = Extension::Doc.header();
        assert_eq!(doc_header, vec![0xd0, 0xcf, 0x11, 0xe0, 0xa1, 0xb1, 0x1a, 0xe1]);
    ...
    fn test_infer_type_from_filename() {
        assert_eq!(FakeFile::infer_type_from_filename(&PathBuf::from("test.pdf")), Extension::Pdf);
        assert_eq!(FakeFile::infer_type_from_filename(&PathBuf::from("test.zip")), Extension::Zip);
        assert_eq!(FakeFile::infer_type_from_filename(&PathBuf::from("test.doc")), Extension::Doc);
        assert_eq!(FakeFile::infer_type_from_filename(&PathBuf::from("test.txt")), Extension::Txt);
        assert_eq!(FakeFile::infer_type_from_filename(&PathBuf::from("test")), Extension::Txt);
        assert_eq!(FakeFile::infer_type_from_filename(&PathBuf::from("test.unknown")), Extension::Txt);
    }
    ...
    fn test_create_buffer_with_header() {
        ...
        assert!(!remaining.iter().all(|&b| b == 0), "Remaining bytes should be random");
    }
    ...
    fn test_create_buffer_txt_file() {
        ...
        assert!(!buffer.iter().all(|&b| b == 0), "All bytes should be random for txt files");
    }
